...
% Meshing
M  = 60;    % Number of points along the circumference
N  = 30;    % Number of points along the radius
% Material for layers
r1=2/3*R;   % Radius of 1st circle (m)
r2=3/4*R;   % Radius of 2nd circle (m)
r3=R;       % Radius of 3rd circle (m)
G1=1;     % Shear modulus of material in 1st circle (Pa)
G2=13e9;     % Shear modulus of material between 1st circle and 2nd circle (Pa)
G3=4e9;     % Shear modulus of material between 2st circle and 3nd circle (Pa)
% Loading
theta=0.001;% rad/m
...
V(1:Ntotal)=struct('Material',[],'G',[],'Part',[],'cont',[],'Vertices',[],'Area',[],'Cent',[],'A',[],'coJi',[],'Ji',[],'Length',[],'n',[],'c5c6',[],'ab',[],'L',[],'Lc',[],'u',[],'W',[],'Jc',[],'strainc',[],'stressc',[]);
...
for order=1:Ntotal
    ...
    if V(order).Cent(1)^2+V(order).Cent(2)^2<=r1^2
        V(order).Material='Null';  % Material 1
        V(order).G=[G1,G1];   
    elseif V(order).Cent(1)^2+V(order).Cent(2)^2<=r2^2
        V(order).Material='Trabecular';  % Material 2
        V(order).G=[G2,G2];   
    elseif V(order).Cent(1)^2+V(order).Cent(2)^2<=r3^2
        V(order).Material='Cordical';  % Material 3
        V(order).G=[G3,G3];
    end
...
for order=1:Ntotal
    ...
    maxstress=max(maxstress,sqrt(V(order).stressc(1)^2+V(order).stressc(2)^2));
end
...
fprintf('          Max stress resultant: %6.4f MPa \n',maxstress/1e6)
...
figure('Name','Material')
h=pcolor(X, Y, Z/10^6);
xlabel('X (m)')
ylabel('Y (m)')
zlabel('Material shear modulus (MPa)')
set(gca,'FontSize',20)
